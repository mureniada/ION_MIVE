...
- Read `docs/CLAUDE_CODE_AUTOMATION_WORKFLOW.md` second, in every session. Role boundary: the handover index is authoritative for **current project state**; the workflow document is authoritative for **bounded Claude Code operating procedure**. Neither overrides the invariants below.
- Current confirmed status (restated here for quick reference — reading the index itself remains required, since this status can change after future approved phases): **M8.1 is CLOSED WITH ACCEPTED EXCEPTION**; **R-001 is CLOSED WITH ACCEPTED PROVENANCE EXCEPTION** (cause NOT ESTABLISHED — closure rests on an accepted provenance exception, not a proven technical cause); **E1 is CLOSED / PASS (5/5)** (architectural thin-client compliance only; the `ui/` versus `frontend/` naming difference remains a separate, non-blocking documentation gap). The **Gemini 401 error and a previously masked OpenAI 401 were both diagnosed and resolved on 2026-08-05** by operator credential replacement, verified by HTTP 200 on deployment `c6c8596d-e4ae-4291-8fd1-6c30938dfef2`; `docs/CANONICAL_SYSTEM_MAP.md` is the canonical record of production identity, resolved incidents, and open defects.
- Read `docs/CANONICAL_SYSTEM_MAP.md` at the start of any infrastructure, deployment, or credential work — it is authoritative for production identity, resolved incidents, and open defects.
